...
public class Dataset {
	 ...
	 * 
	 * @param d 
	 * @param outputSize Percentage to be extracted from d
	 * @param seed
	 * @return
	 */
	public static Dataset getSubsetFromDataset (Dataset inputDataset, int outputSize, int seed) {
		Dataset output;
		
		final Resample filter = new Resample();
		  Instances filteredIns = null;
		  try {
		    filter.setInputFormat(inputDataset.getInstances());
		    filter.setNoReplacement(true);
		    filter.setSampleSizePercent(outputSize);
		    filter.setSeed(seed);
		    filteredIns = Filter.useFilter(inputDataset.getInstances(), filter);
		  } catch (Exception e) {
		    System.out.println("Error when resampling input data!");
		    e.printStackTrace();
		  }
		  output = new Dataset(filteredIns);
		return output;	
	}
